...
async fn dispatch_hyper_downstream_request(
    state: SharedState,
    tracker: DownstreamHttp2ConnectionTracker,
    ...
    let on_upgrade = request_supports_upgrade(&request).then(|| hyper::upgrade::on(&mut request));
    ...
    S: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin + Send + 'static,
{
    ...
    let service = {
        ...
        hyper::service::service_fn(move |request: hyper::Request<Incoming>| {
            ...
            dispatch_hyper_downstream_request(state, tracker, connection_metadata, request)
        })
    };
...
pub(super) async fn serve_http1_connection_via_hyper<S>(
    state: SharedState,
    stream: S,
    peer_addr: SocketAddr,
    connection_metadata: Option<DownstreamConnectionMetadata>,
            ...
            async move {
                ...
                let captured_request = CapturedPromotedHttpRequest {
                    request: build_downstream_http_request_context(
                        LazyRequestId::from_string(request_id),
                        parts,
                        body,
