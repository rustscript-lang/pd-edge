...
- [`trace.rs`](../rustscript/src/vm/jit/trace.rs)
- [`runtime.rs`](../rustscript/src/vm/jit/runtime.rs)
- [`codegen.rs`](../rustscript/src/vm/jit/native/codegen.rs)
- [`cranelift.rs`](../rustscript/src/vm/jit/native/cranelift.rs)
- [`bridge.rs`](../rustscript/src/vm/jit/native/bridge.rs)
- [`core.rs`](../rustscript/src/builtins/runtime/core.rs)
- [`bytes.rs`](../rustscript/src/builtins/runtime/bytes.rs)
- [`layout.rs`](../rustscript/src/vm/native/layout.rs)
- [`bytecode.rs`](../rustscript/src/bytecode.rs)
- [`mod.rs`](../rustscript/src/vm/mod.rs)
- [`jit_tests.rs`](../rustscript/tests/jit/jit_tests.rs)
...
- The VM already has `string_concat_op()` and `bytes_concat_op()` in
  [`mod.rs`](../rustscript/src/vm/mod.rs), so interpreter semantics exist for both concat cases.
- The trace recorder only gives `string + string` a dedicated trace step today. In
  [`trace.rs`](../rustscript/src/vm/jit/trace.rs), `OpCode::Add` lowers to `TraceStep::SConcat` for
...
- Native codegen explicitly declines to inline `TraceStep::BuiltinCall` in
  [`codegen.rs`](../rustscript/src/vm/jit/native/codegen.rs), so these operations never get dedicated
  native lowering.
...
1. Refactor typed string/bytes representation in [`trace.rs`](../rustscript/src/vm/jit/trace.rs):
   - add `TraceTextBytesKind::{String, Bytes}`
   - replace `TraceStep::SConcat` with `TraceStep::Concat(TraceConcatKind)`
   - add `TraceStep::Len(TraceTextBytesKind)`
   - add `TraceStep::Slice(TraceTextBytesKind)`
...
3. Add explicit VM-side string/bytes op methods in [`mod.rs`](../rustscript/src/vm/mod.rs):
   - `string_len_op()`
   - `bytes_len_op()`
   - `string_slice_op()`
   - `bytes_slice_op()`
   ...
   - keep them aligned with the current builtin semantics in
     [`core.rs`](../rustscript/src/builtins/runtime/core.rs) and
     [`bytes.rs`](../rustscript/src/builtins/runtime/bytes.rs)

4. Add the native data-path support needed by [`codegen.rs`](../rustscript/src/vm/jit/native/codegen.rs):
   - use the existing probed value layout from [`layout.rs`](../rustscript/src/vm/native/layout.rs)
...
5. Replace helper-based concat lowering in [`codegen.rs`](../rustscript/src/vm/jit/native/codegen.rs)
...
8. Update [`cranelift.rs`](../rustscript/src/vm/jit/native/cranelift.rs):
...
9. Clean up obsolete concat-helper code in [`bridge.rs`](../rustscript/src/vm/jit/native/bridge.rs):
...
10. Keep the trace interpreter aligned in [`runtime.rs`](../rustscript/src/vm/jit/runtime.rs):
...
Add or update tests in [`jit_tests.rs`](../rustscript/tests/jit/jit_tests.rs).
