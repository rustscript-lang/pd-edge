...
- [`bridge.rs`](../rustscript/src/vm/native/bridge.rs)
...
- `array + array` is not a typed collection op in the trace recorder today. It stays as generic
  `TraceStep::Add` in [`trace.rs`](../rustscript/src/vm/jit/trace.rs).
- In native JIT, generic `TraceStep::Add` enters the int fast path in
  [`codegen.rs`](../rustscript/src/vm/jit/native/codegen.rs), and non-int operands fall back to the
...
- Native codegen explicitly declines to inline `TraceStep::BuiltinCall` in
  [`codegen.rs`](../rustscript/src/vm/jit/native/codegen.rs), so these operations never get a dedicated
  native lowering.
...
1. Add typed collection steps in [`trace.rs`](../rustscript/src/vm/jit/trace.rs):
...
3. Add explicit VM-side collection op methods in [`mod.rs`](../rustscript/src/vm/mod.rs):
   - `array_new_op()`
   - `map_new_op()`
   - `array_push_op()`
...
5. Lower typed collection steps in [`codegen.rs`](../rustscript/src/vm/jit/native/codegen.rs):
...
6. Update [`cranelift.rs`](../rustscript/src/vm/jit/native/cranelift.rs):
...
7. Keep the trace interpreter aligned in [`runtime.rs`](../rustscript/src/vm/jit/runtime.rs):
